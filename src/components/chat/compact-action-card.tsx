...
const TYPE_CONFIG: Record<string, { icon: typeof Wine; color: string; label: string }> = {
  ...
  transfer_receive: { icon: Package, color: 'orange', label: 'โอนสต๊อก' },
...
export const CompactActionCard = memo(function CompactActionCard({ message }: CompactActionCardProps) {
  ...
  const config = TYPE_CONFIG[meta.action_type] || TYPE_CONFIG.generic;
  const Icon = config.icon;
  const statusInfo = STATUS_ICON[meta.status] || STATUS_ICON.pending;
  ...
  if (meta.reference_id) summaryParts.push(`#${meta.reference_id.slice(-8)}`);
  if (meta.summary?.customer) summaryParts.push(meta.summary.customer);
  if (meta.summary?.items) summaryParts.push(meta.summary.items);
  const summaryText = summaryParts.join(' · ') || config.label;
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <span className={cn('text-[10px] font-medium', statusInfo.className)}>
          {meta.status === 'claimed' && meta.claimed_by_name
