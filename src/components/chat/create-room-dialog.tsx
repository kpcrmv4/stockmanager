...
const ROOM_TYPE_OPTIONS: { value: CreatableRoomType; label: string; description: string; icon: React.ComponentType<{ className?: string }>; ownerOnly?: boolean }[] = [
  {
    value: 'dm',
    label: '1:1',
...
export function CreateRoomDialog({ isOpen, onClose }: CreateRoomDialogProps) {
  ...
  const handleCreate = async () => {
    ...
    const dbRoomType = roomType === 'dm' ? 'direct' : roomType;
